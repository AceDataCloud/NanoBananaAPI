...
When you want to edit an existing image, set `action` to `edit`, and pass the list of image URLs to be edited through `image_urls` (one or more), while also providing a `prompt` that describes the editing goal.
...
**Callback Payload Example** (field structure is consistent with synchronous success response):
