...
When you want to edit based on existing images, set `action` to `edit`, and pass the list of image URLs to be edited through `image_urls` (one or more), while also providing a `prompt` describing the editing goal.
